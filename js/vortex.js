...
Tunnel.prototype.init = function () {
    ...
    this.camera = new THREE.PerspectiveCamera(15, ww / wh, 0.02, 100);
    this.camera.rotation.y = Math.PI;
    this.camera.position.z = 0.35;

    this.scene = new THREE.Scene();
    this.scene.fog = new THREE.Fog(0xffffff, 0, 1.8);
...
Tunnel.prototype.addParticle = function () {
    this.particles = [];
    for (var i = 0; i < (isMobile ? 30 : 70); i++) {
...
Tunnel.prototype.updateCurve = function () {
    ...
    this.curve.points[2].x = 0;
    this.curve.points[3].x = 0;
    this.curve.points[4].x = 0;

    this.curve.points[2].y = 0;
    this.curve.points[3].y = 0;
    this.curve.points[4].y = 0;
...
Tunnel.prototype.render = function (time) {
    ...
    var currentScrollValue = window.pageYOffset / (document.documentElement.scrollHeight - window.innerHeight);
    if (currentScrollValue > scroll) {
        this.speed = 0.4
    } else if (currentScrollValue < scroll) {
        this.speed = -0.4
    } else {
        this.speed = 0
    }
    scroll = currentScrollValue
    this.camera.position.z = scroll * 1.8
    this.scene.fog.far = 1.8 - scroll * 1.8
